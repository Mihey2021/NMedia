...
class EditPostResultContract : ActivityResultContract<Post?, Pair<Post?, String?>?>() {
    override fun createIntent(context: Context, input: Post?): Intent =
        Intent(context, NewPostFragment::class.java)
            .putExtra(NewPostFragment.EDIT_POST, input)


    override fun parseResult(resultCode: Int, intent: Intent?): Pair<Post?, String?>? =
        if (resultCode == Activity.RESULT_OK) {
            intent?.getSerializableExtra(NewPostFragment.EDIT_POST) as? Post to intent?.getStringExtra(
                NewPostFragment.POST_CONTENT
            )
        } else {
            null
        }
